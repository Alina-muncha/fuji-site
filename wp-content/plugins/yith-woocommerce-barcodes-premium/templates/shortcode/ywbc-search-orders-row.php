...
<tr class="ywbc-search-row">
	...
	<td valign="top">
		...
		foreach ( $order->get_items() as $item ) {
			...
			$product_images[] = wp_get_attachment_image_src( get_post_thumbnail_id( $item->get_product_id() ), 'thumbnail' );
			$product_names[] = $product_quantity . ' x ' . '<a href="' . $url . '"> ' . $product_name ;
		}
		$cnt = 0;
		foreach ( $product_names as $product_name ){
			echo '<div>' . $product_name . '<br>';

			?>
			<img src="<?php  echo $product_images[$cnt][0]; ?>" data-id="<?php echo $item->get_product_id(); ?>">
