...
final class RWS_DAT_IMPORTER {
	...
	public function init( $wp_customize, $args )
	{
		if ( current_user_can( 'edit_theme_options' ) ) {
				...
				$this->rws_import( $wp_customize, $args );
		}
	}
	 ...
	 * @return void
	 */
	public function rws_import( $wp_customize, $args )
	{
		...
		// Get the upload data.
		$dat_file_path = RWS_DEMO_CONTENT_PATH.$args['dat_file'];
		$raw  = file_get_contents( $dat_file_path );
		$data = @unserialize( $raw );

		// Import images.
		
			$data['mods'] = $this->rws_import_images( $data['mods'] );
		...
		if ( isset( $data['options'] ) ) {
			...
			foreach ( $data['options'] as $option_key => $option_value ) {
				$option = new RWS_DAT_Option( $wp_customize, $option_key, array(
					'default'		=> '',
					'type'			=> 'option',
	 ...
	 */
	private function rws_import_images( $mods )
	{
		foreach ( $mods as $key => $val ) {

			if ( $this->rws_is_image_url( $val ) ) {

				$data = $this->rws_sideload_image( $val );
	 ...
	 */
	private function rws_sideload_image( $file )
	{
		$data = new stdClass();
	 ...
	 */
	private function rws_is_image_url( $string = '' )
	{
		if ( is_string( $string ) ) {
...
function rws_dat_import($args){

$test = new RWS_DAT_IMPORTER();
$test->init ( $wp_customize, $args );
