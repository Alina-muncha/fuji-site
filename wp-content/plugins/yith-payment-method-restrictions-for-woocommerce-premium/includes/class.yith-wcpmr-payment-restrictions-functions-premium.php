    ...
    class YITH_Payment_Restrictions_Functions_Premium extends YITH_Payment_Restrictions_Functions
    {
        ...
        function is_payment_method_disabled( $gateway,$rules,$order_cart = '' ) {
            ...
            foreach ( $rules as $rule ) {
                ...
                foreach ( $conditions as $condition ) {
                    ...
                    if ( isset( $condition['products_selected']  ) && is_array( $condition['products_selected'] ) ) {
						$product_selected_simple_and_variations = array();
						// Check on product_selected if there is a variable product, add it's variations.
						foreach ( $condition['products_selected'] as $product_selected ) {
							$product = wc_get_product( $product_selected );
							if ( $product && $children = $product->get_children() ) {
								$product_selected_simple_and_variations = array_merge( $product_selected_simple_and_variations, $children );
							}
						}

						$condition['products_selected'] = array_merge( $condition['products_selected'], $product_selected_simple_and_variations );
					}
                    ...
                    switch($condition['type_restriction']) {
                        ...
                        case 'geolocalization' :
                            ...
                            $remove_payment_method = $this->restriction_by_geolocalization($condition['restriction_by'],$condition['geolocalization'], $gateway);
                            break;
						...
						case 'user' :
							$remove_payment_method = $this->restriction_by_user($condition['restriction_by'],$condition['users_selected']);

							break;
						default :
                            $remove_payment_method = apply_filters('yith_wcpmr_is_payment_method_disabled',false,$condition['type_restriction'],$condition,$gateway,$order_cart);
                            break;
                    }
                }
        ...
        function restriction_by_products( $restriction_by,$selected_products,$order_cart ) {
            ...
            if ( ( is_checkout() && !is_wc_endpoint_url( 'order-pay' ) ) || apply_filters('yith_wcpmr_restriction_by',false) ) {
				...
				if( !$item_cart && isset( $wp->query_vars['order-received'] ) ) {
         ...
         * @return boolean
         */
        function restriction_by_geolocalization($restriction_by,$countries, $gateway = false) {

            $geolocalization_type = apply_filters('yith_wcpmr_geolocalization_type', get_option('yith_wcpmr_get_geolocalization_how_work', true ), $gateway );

            $country = '';


            switch ( $geolocalization_type ) {
                case 'ip':
                ...
                case 'billing_country':
                        ...
                        $country =( empty($country) && isset( $_POST['billing_country']) ) ? $_POST['billing_country'] : $country;
		 ...
		 * @return boolean
		 */
		function restriction_by_user( $restriction_by,$selected_users ) {
			...
			switch( $restriction_by ) {
				...
				if( ! empty( $selected_users ) && ! empty( $user ) ) {
					if( ! empty( $selected_users ) && ! empty( $user ) ){
