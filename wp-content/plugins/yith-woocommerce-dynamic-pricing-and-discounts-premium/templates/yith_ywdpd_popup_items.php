	...
	foreach ( $item_ids as $item_id ) :
		if ( 'product' === $item_class ) {
			...
			if ( $object->is_in_stock() ) {
				...
				$row .= '</div>';
				$row         .= sprintf( "<span class='single_add_to_cart_button button %s'>%s</span>",$object->get_type(), $object->add_to_cart_text() );
				$row .= '</div>';
			}
		} else {
			$data     = '';
		...
		?>
		<li class="<?php echo esc_attr( $item_class ); ?> item" <?php echo $data; ?> >
			<?php echo $row; ?>
