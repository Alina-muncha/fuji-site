    ...
    */
    abstract class THMAF_Admin_Settings {
        ...
        public function __construct($page, $section = '') {
            $this->page_id = $page;
            $this->tabs = array('general_settings' => esc_html__('General Settings', 'themehigh-multiple-addresses'));
        }
        ...
        public function render_form_field_element($field, $atts = array(), $render_cell = true) {
            if($field && is_array($field)) {
                ...
                if($render_cell) {
                    ...
                    if($sub_label) { ?>
                        <td <?php echo esc_html($label_cell_props) ?> >
                            <?php echo esc_html($flabel); echo esc_html($required_html); ?>
                                <br/><span class="thpladmin-subtitle"><?php echo esc_html($sub_label); ?></span>
                        </td>
                    <?php } ?>
                    <td <?php echo esc_html($input_cell_props) ?> ><?php echo $field_html; ?></td>
                <?php }else {
                    echo $field_html;
                }
            }
        ...
        private function _prepare_form_field_props($field, $atts = array()) {
            ...
            $field_props  = 'name="'. $fname .'" value="'. esc_attr($fvalue) .'" style="'. $input_width .'"id="'.$fid.'"';
            ...
            if($field && is_array($field)) {
                ...
                if(!empty($field['options']) && is_array($field['options'])){
                    foreach($field['options'] as $value => $label){
                        ...
                        $field_html .= '<option value="'. trim($value) .'" '.$selected.'>'. THMAF_i18n::__t($label) .'</option>';
                    }
                }
                $field_html .= '</select>';
            }
